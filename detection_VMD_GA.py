...
def sample_z(args):
    ...
    # epsilon = tf.random.normal(shape=(10, 6), mean=0.0, stddev=1.0)
    # coverage rate thresholds=50
    epsilon = tf.random.normal(shape=(10, 16), mean=0.0, stddev=1.0)
    # coverage rate thresholds=30
    # epsilon = tf.random.normal(shape=(10, 32), mean=0.0, stddev=1.0)
...
def plot_log_likelihood(df_log_px):
    ...
    # plt.savefig('output/VMD/GA/log_likelihood_VMD_IFS_'+id+'.png')
def main():
    # coverage rate thresholds=70
    # all_df = pd.read_csv('dataset/VMD/limo_relation_data_6.14_2.csv', index_col='TIMESTAMP',
    #                      usecols=['TIMESTAMP', 'L0039','L0042','L0138','L0140','L0141','L0147'])
    # coverage rate thresholds=50
    all_df = pd.read_csv('dataset/VMD/limo_relation_data_6.14_2.csv', index_col='TIMESTAMP',
                         usecols=['TIMESTAMP', 'ZD_P', 'L0016','L0019','L0039','L0040', 'L0042','L0050','L0055',
                                  'L0056', 'L0069','L0070','L0138','L0140','L0141','L0142','L0147'])
    # coverage rate thresholds=30
    # all_df = pd.read_csv('dataset/VMD/limo_relation_data_6.14_2.csv', index_col='TIMESTAMP',
    #                      usecols=['TIMESTAMP', 'ZD_P', 'L0014','L0016','L0018','L0019','L0020','L0039','L0040', 'L0041', 'L0042','L0050','L0052','L0054','L0055',
    #                               'L0056','L0057','L0058','L0059','L0064', 'L0069','L0070','L0136','L0137','L0138','L0139','L0140','L0141','L0142','L0143','L0144',
    #                               'L0145','L0147'])
    ...
    # plt.savefig('output/VMD/GA/anomaly_lstm_vae_VMD_RFE_'+id+'.png')
    # df_log_px.to_csv("output/VMD/GA/Anomaly_lstm_vae_VMD_GA_70_"+id+".csv")
    df_log_px.to_csv("output/VMD/GA/Anomaly_lstm_vae_VMD_GA_50_" + id + ".csv")
    # df_log_px.to_csv("output/VMD/GA/Anomaly_lstm_vae_VMD_GA_30_" + id + ".csv")
