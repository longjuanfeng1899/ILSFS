...
def sample_z(args):
    ...
    # epsilon = tf.random.normal(shape=(10, 8), mean=0.0, stddev=1.0)
...
def plot_log_likelihood(df_log_px):
    ...
    # plt.savefig('output/VMD/RFE/log_likelihood_VMD_IFS_'+id+'.png')
def main():
    # coverage rate thresholds=70
    # all_df = pd.read_csv('dataset/VMD/limo_relation_data_6.14_2.csv', index_col='TIMESTAMP',
    #                      usecols=['TIMESTAMP', 'ZD_P','L0040','L0049','L0069','L0070','L0147'])
    # coverage rate thresholds=50
    # all_df = pd.read_csv('dataset/VMD/limo_relation_data_6.14_2.csv', index_col='TIMESTAMP',
    #                      usecols=['TIMESTAMP', 'ZD_P', 'L0014', 'L0040', 'L0049',
    #                               'L0064', 'L0069', 'L0070', 'L0147'])
    # coverage rate thresholds=30
    all_df = pd.read_csv('dataset/VMD/limo_relation_data_6.14_2.csv', index_col='TIMESTAMP',
                         usecols=['TIMESTAMP', 'ZD_P', 'L0014','L0040', 'L0041', 'L0049',
                                  'L0064', 'L0069','L0070', 'L0140','L0147'])
    ...
    # plt.savefig('output/VMD/RFE/anomaly_lstm_vae_VMD_RFE_'+id+'.png')
    # df_log_px.to_csv("output/VMD/RFE/Anomaly_lstm_vae_VMD_RFE_70_"+id+".csv")
    # df_log_px.to_csv("output/VMD/RFE/Anomaly_lstm_vae_VMD_RFE_50_" + id + ".csv")
    df_log_px.to_csv("output/VMD/RFE/Anomaly_lstm_vae_VMD_RFE_30_" + id + ".csv")
