...
def sample_z(args):
    ...
    epsilon = tf.random.normal(shape=(10, 38), mean=0.0, stddev=1.0)
...
def plot_log_likelihood(df_log_px):
    ...
    # plt.savefig('output/SMD/log_likelihood_'+id+'.png')
def main():
    all_df = pd.read_csv('dataset/SMD/machine-1-1.csv', index_col='timestamp')
    # all_df.index = pd.to_datetime(all_df.index, unit='ms') 28479
    ...
    # plt.savefig('output/SMD/anomaly_lstm_vae_SMD_'+id+'.png')
    df_log_px.to_csv("output/SMD/Anomaly_lstm_vae_SMD_0_"+id+".csv")
